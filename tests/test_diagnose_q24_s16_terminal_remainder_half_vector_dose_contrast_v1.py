"""Independent exact half-dose/suffix oracle and authentication/guard mutations."""
...
from ace3.model.candidates import diagnose_q24_s16_terminal_remainder_half_vector_dose_contrast_v1 as d


EVIDENCE = REPORT = OUTPUTS = IDENTITY = DELTA = SUMMARIES = None
spec = importlib.util.spec_from_file_location("reviewed_half_dose_suffix_oracle", d.closed.TEST)
...
class HalfDoseTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        if EVIDENCE is None:
            raise RuntimeError("run native --check for authenticated nonzero focused tests")

    def test_independent_integer_suffix_oracle_both_polarities_all_controls(self):
    ...
    def test_all896_half_targets_input_order_and_independent_rne(self):
        ...
        for (control, polarity), output in OUTPUTS.items():
            ...
            targets = [oracle.q(a) + sign * b / 2 for a, b in zip(
            ...
            self.assertEqual(operand["dose"], "1/2")
            self.assertEqual(operand["signed_dose"], str(Fraction(sign, 2)))
    ...
    def test_zero_entries_subnormals_ties_even_and_single_conversion(self):
        words = np.full(896, 0x3c00, dtype="<u2")
        words[:4] = [0, 0x8000, 0x3c00, 0x3c01]
        delta = (Fraction(1, 1 << 24), Fraction(-1, 1 << 24),
                 Fraction(1, 1024), Fraction(1, 1024)) + (Fraction(),) * 892
        for polarity, sign in (("forward", -1), ("reverse", 1)):
            result, operand = d.prepare(words, delta, polarity)
            expected = [oracle.nearest(oracle.q(a) + sign * b / 2)
        ...
        reverse, _ = d.prepare(words, delta, "reverse")
        self.assertEqual(reverse[2:4].tolist(), [0x3c00, 0x3c02])

    def test_selection_dose_order_union_pair_and_polarity_mutations(self):
    ...
    def test_retained_comparators_and_closed_outcome_mutations(self):
        self.assertNotIn("report", EVIDENCE)
        for index, key, value in (
                (0, "status", "rejected"), (1, "status", "UNKNOWN"),
                (1, "protected_input_identity", "foreign"),
                (1, "retained_common_component", "PASS"),
                (1, "prediction_receipt", {}), (0, "single_stdout_json", False),
                (1, "contrast_count", 35)):
        ...
        for index in (0, 1):
            for key in ("predicted_delta", "observed_delta", "full_vector_linear_bridge_delta"):
                summaries = deepcopy(SUMMARIES)
            ...
            with self.assertRaises(ValueError):
                d.bind_doses(EVIDENCE, *summaries)
        summaries = deepcopy(SUMMARIES)
        summaries[1]["contrasts"][0]["retained_margin_change"] = "0"
        with self.assertRaises(ValueError):
            d.bind_doses(EVIDENCE, *summaries)

    def test_fixed_references_obstructions_and_same_polarity_full_dose(self):
        forward = {(r["control"], *r["pair"]): r for r in SUMMARIES[0]["contrasts"]}
        reverse = {(r["control"], r["left_id"], r["right_id"]): r
                   for r in SUMMARIES[1]["contrasts"]}
        ...
        for row, obstruction in zip(REPORT["contrasts"],
            ...
            key = row["control"], row["left_id"], row["right_id"]
            retained = (forward if row["polarity"] == "forward" else reverse)[key]
            self.assertEqual(Fraction(row["predicted_delta"]),
                             Fraction(retained["predicted_delta"]) / 2)
            self.assertEqual(row["same_polarity_full_dose_delta"], retained["observed_delta"])
            ...
            self.assertEqual(Fraction(row["retained_actual_margin"]),
                             oracle.q(actual[key[1]]) - oracle.q(actual[key[2]]))
            self.assertEqual(Fraction(row["fixed_reference_margin"]),
                             oracle.q(ref[key[1]]) - oracle.q(ref[key[2]]))
    ...
    def test_strict_dose_classification_zero_opposite_equal_and_overshoot(self):
        rows = [{**r, "observed_delta": str(Fraction(r["same_polarity_full_dose_delta"]) / 2)}
                for r in REPORT["contrasts"]]
        self.assertEqual(d.classify(rows), "supported")
        for factor in (0, -1, 1, 2):
            changed = deepcopy(rows)
            changed[0]["observed_delta"] = str(factor * Fraction(
                changed[0]["same_polarity_full_dose_delta"]))
    ...
    def test_source_test_review_receipt_and_current_pin_mutations(self):
        for module, mission, source, test, review in (
                *d.closed.CHAIN, d.vector.CLOSED_CHAIN, d.complement.VECTOR_CHAIN,
                d.reverse.COMPLEMENT_CHAIN, d.REVERSE_CHAIN):
        ...
        for path in (d.SOURCE, d.TEST, d.REVERSE_RECEIPT["path"],
                     d.reverse.PREDICTION_RECEIPT["path"]):
            with self.assertRaises(ValueError):
                d.base.read_bound({"path": str(path), "sha256": "0" * 64})
        with patch.object(d, "REVERSE_CALL", "missing"), self.assertRaises(ValueError):
            d.retained_doses()
    ...
    def test_forbidden_ranking_union_closed_replay_prefix_and_writes(self):
        ...
        destinations = (d.SOURCE, d.TEST, d.REVERSE_RECEIPT["path"],
    ...
    def test_one_stdout_json_unknown_and_non_admission_closed_flags(self):
        ...
        for key, value in d.reverse.FLAGS.items():
        ...
        for key in ("candidate_admitted", "strict_FP16_state_claim", "new_token_claim",
                    ...
                    "closed_reverse_vector_intervention_replay", "full_dose_recomputation",
                    "dose_search"):
        ...
        self.assertEqual(len(REPORT["closed_results"]), 4)
