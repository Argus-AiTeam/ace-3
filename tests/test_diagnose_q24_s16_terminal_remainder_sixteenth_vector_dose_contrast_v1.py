"""Independent sixteenth-dose conversion/suffix oracle and integrity mutations."""
...
from ace3.model.candidates import diagnose_q24_s16_terminal_remainder_sixteenth_vector_dose_contrast_v1 as d


EVIDENCE = REPORT = OUTPUTS = IDENTITY = DELTA = SUMMARIES = None
spec = importlib.util.spec_from_file_location("reviewed_sixteenth_suffix_oracle", d.closed.TEST)
...
class SixteenthDoseTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        if EVIDENCE is None:
    ...
    def test_exact896_sixteenth_targets_and_independent_rne(self):
        ...
        for (control, polarity), output in OUTPUTS.items():
            ...
            targets = [oracle.q(a) + sign * b / 16 for a, b in zip(
            ...
            self.assertEqual(operand["dose"], "1/16")
            self.assertEqual(operand["signed_dose"], str(Fraction(sign, 16)))
    ...
    def test_zero_subnormal_ties_even_and_no_rounded_eighth_averaging(self):
        ...
        rounded_eighth = np.array([oracle.nearest(actual + delta[3] / 8)], dtype="<u2")
        averaged = oracle.nearest((actual + oracle.q(rounded_eighth.view("<f2")[0])) / 2)
    ...
    def test_selection_order_pair_polarity_and_dose_search_refused(self):
        ...
        for name, value in (("COORDINATES", (363,)), ("COORDINATES", d.COORDINATES[::-1]),
                            ...
                            *(("DOSE", v) for v in (
                                Fraction(1, 8), Fraction(1, 32), Fraction(1, 4),
                                Fraction(1, 2), Fraction(1), 0.0625))):
    ...
    def test_closed_receipt_outcome_census_and_comparator_mutations(self):
        mutations = [(i, "status", "UNKNOWN") for i in range(5)]
        mutations += [(i, "protected_input_identity", "foreign") for i in range(1, 5)]
        mutations += [(4, k, v) for k, v in (
        ...
        for index in range(4):
            for key in ("predicted_delta", "observed_delta"):
                ...
                with self.assertRaises(ValueError):
                    d.bind_doses(EVIDENCE, *summaries)
        signatures = SUMMARIES[4]["distinct_forward_pair_movements"]
        mutations = [signatures[:-1], signatures[::-1], [signatures[0]] * 4]
        for index in range(1, 5):
            bad = deepcopy(signatures)
            bad[0][index] = "0"
            mutations.append(sorted(bad))
        ambiguous = deepcopy(signatures)
        ambiguous[1] = [signatures[0][0], "-1/64", *signatures[0][2:]]
        mutations.append(sorted(ambiguous))
        for bad in mutations:
            summaries = deepcopy(SUMMARIES)
            summaries[4]["distinct_forward_pair_movements"] = bad
            with self.assertRaises(ValueError):
                d.bind_doses(EVIDENCE, *summaries)
    ...
    def test_source_test_review_receipt_and_current_pin_mutations(self):
        for module, mission, source, test, review in (
                *d.closed.CHAIN, d.vector.CLOSED_CHAIN, d.complement.VECTOR_CHAIN,
                ...
                d.quarter.HALF_CHAIN, d.eighth.QUARTER_CHAIN, d.EIGHTH_CHAIN):
        ...
        for path in (d.SOURCE, d.TEST, *(r["path"] for r in d.RECEIPTS)):
            with self.assertRaises(ValueError):
                d.base.read_bound({"path": str(path), "sha256": "0" * 64})
        with patch.object(d, "EIGHTH_CALL", "missing"), self.assertRaises(ValueError):
            d.retained_doses()
    ...
    def test_forbidden_ranking_union_closed_replay_prefix_and_writes(self):
        ...
        for module in (d.closed, d.vector, d.complement, d.reverse, d.half, d.quarter, d.eighth):
        ...
        destinations = (d.SOURCE, d.TEST, d.EIGHTH_RECEIPT["path"],
    ...
    def test_strict_sixteenth_classification_zero_opposite_equal_and_overshoot(self):
    ...
    def test_fixed_reference_obstructions_and_unique_eighth_signature(self):
        signatures = SUMMARIES[4]["distinct_forward_pair_movements"]
        quarter_rows = {(r["control"], r["polarity"], r["left_id"], r["right_id"]): r
                        for r in SUMMARIES[3]["contrasts"]}
        used = set()
        ...
        for row, obstruction in zip(REPORT["contrasts"],
            ...
            retained = quarter_rows[row["control"], row["polarity"], row["left_id"], row["right_id"]]
            self.assertEqual(Fraction(row["predicted_delta"]), Fraction(retained["predicted_delta"]) / 4)
            expected = [Fraction(retained["observed_delta"]),
                        Fraction(retained["same_polarity_half_dose_delta"]),
                        Fraction(retained["same_polarity_full_dose_delta"])]
            self.assertEqual([Fraction(row[k]) for k in d.DOSE_KEYS[1:]], expected)
            sign = 1 if row["left_id"] == 319 else -1
            matches = [s for s in signatures if s[0] == row["polarity"]
                       and [sign * Fraction(v) for v in s[2:]] == expected]
            self.assertEqual(len(matches), 1)
            self.assertEqual(Fraction(row[d.DOSE_KEYS[0]]), sign * Fraction(matches[0][1]))
            used.add(tuple(matches[0]))
            self.assertEqual(row["eighth_receipt_pair"], [319, 34319])
            self.assertEqual(row["eighth_comparator_signature"], [matches[0][0], *matches[0][2:]])
            self.assertEqual(row["eighth_comparator_derivation"], "unique_closed_dose_signature"
        ...
        self.assertEqual(used, set(map(tuple, signatures)))
    ...
    def test_one_stdout_json_unknown_and_closed_non_admission_flags(self):
        ...
        for key, value in d.eighth.FLAGS.items():
        ...
        for key in ("candidate_admitted", "strict_FP16_state_claim", "new_token_claim",
                    ...
                    "full_dose_recomputation", "half_dose_recomputation",
                    "quarter_dose_recomputation", "eighth_dose_recomputation", "dose_search"):
        ...
        self.assertEqual(len(REPORT["closed_results"]), 7)
