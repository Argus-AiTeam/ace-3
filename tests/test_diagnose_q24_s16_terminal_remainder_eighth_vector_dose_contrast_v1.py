"""Independent eighth-dose conversion/suffix oracle and integrity mutations."""
...
from ace3.model.candidates import diagnose_q24_s16_terminal_remainder_eighth_vector_dose_contrast_v1 as d


EVIDENCE = REPORT = OUTPUTS = IDENTITY = DELTA = SUMMARIES = None
spec = importlib.util.spec_from_file_location("reviewed_eighth_suffix_oracle", d.closed.TEST)
...
class EighthDoseTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        if EVIDENCE is None:
    ...
    def test_exact896_eighth_targets_and_independent_rne(self):
        ...
        for (control, polarity), output in OUTPUTS.items():
            ...
            targets = [oracle.q(a) + sign * b / 8 for a, b in zip(
            ...
            self.assertEqual(operand["dose"], "1/8")
            self.assertEqual(operand["signed_dose"], str(Fraction(sign, 8)))
    ...
    def test_zero_subnormal_ties_even_and_no_rounded_quarter_averaging(self):
        words = np.full(896, 0x3c00, dtype="<u2")
        words[:5] = [0, 0x8000, 0x3c00, 0x3c01, 0x3c01]
        delta = (Fraction(1, 1 << 22), Fraction(-1, 1 << 22),
                 Fraction(1, 256), Fraction(1, 512), Fraction(1, 256)) + (Fraction(),) * 891
        for polarity, sign in (("forward", -1), ("reverse", 1)):
            result, operand = d.prepare(words, delta, polarity)
            self.assertEqual(result.tolist(), [
                oracle.nearest(oracle.q(a) + sign * b / 8)
        ...
        rounded_quarter = np.array([oracle.nearest(actual + delta[3] / 4)], dtype="<u2")
        averaged = oracle.nearest((actual + oracle.q(rounded_quarter.view("<f2")[0])) / 2)
    ...
    def test_selection_order_pair_polarity_and_dose_search_refused(self):
        ...
        for name, value in (("COORDINATES", (363,)), ("COORDINATES", d.COORDINATES[::-1]),
                            ...
                            ("DOSE", Fraction(1, 4)), ("DOSE", Fraction(1, 16)),
                            ("DOSE", Fraction(1, 2)), ("DOSE", Fraction(1)),
                            ("DOSE", 0.125)):
    ...
    def test_closed_receipt_outcome_census_and_comparator_mutations(self):
        for index, key, value in (
                ...
                (3, "status", "rejected"), (3, "protected_input_identity", "foreign"),
                (3, "retained_common_component", "PASS"), (3, "native_exit", 1),
                (3, "stdout_json_documents", 2), (3, "strict_dose_agreements", 71),
                (3, "directional_agreements", 71), (3, "contrast_count", 71),
                (3, "tests", {"executed": 0, "errors": 0, "failures": 0, "skipped": 0}),
                (3, "dispatch_and_write_audit", {})):
        ...
        for index in range(4):
            ...
            with self.assertRaises(ValueError):
                d.bind_doses(EVIDENCE, *summaries)
        for key in ("same_polarity_full_dose_delta", "same_polarity_half_dose_delta",
                    "retained_margin_change", "intervened_margin_change"):
            for index in (0, 1):
                summaries = deepcopy(SUMMARIES)
                summaries[3]["contrasts"][index][key] = "0"
                with self.assertRaises(ValueError):
                    d.bind_doses(EVIDENCE, *summaries)
    ...
    def test_source_test_review_receipt_and_current_pin_mutations(self):
        for module, mission, source, test, review in (
                *d.closed.CHAIN, d.vector.CLOSED_CHAIN, d.complement.VECTOR_CHAIN,
                ...
                d.quarter.HALF_CHAIN, d.QUARTER_CHAIN):
        ...
        for path in (d.SOURCE, d.TEST, *(r["path"] for r in d.RECEIPTS)):
            with self.assertRaises(ValueError):
                d.base.read_bound({"path": str(path), "sha256": "0" * 64})
        with patch.object(d, "QUARTER_CALL", "missing"), self.assertRaises(ValueError):
            d.retained_doses()
    ...
    def test_forbidden_ranking_union_closed_replay_prefix_and_writes(self):
        ...
        destinations = (d.SOURCE, d.TEST, d.QUARTER_RECEIPT["path"],
    ...
    def test_strict_eighth_classification_zero_opposite_equal_and_overshoot(self):
        rows = [{**r, "observed_delta": str(Fraction(r["same_polarity_quarter_dose_delta"]) / 2)}
                for r in REPORT["contrasts"]]
        self.assertEqual(d.classify(rows), "supported")
        for key in ("same_polarity_quarter_dose_delta", "same_polarity_half_dose_delta",
        ...
        for bad in (rows[:-1], rows[::-1], [rows[0]] * 72,
                    ...
                    [{**r, "same_polarity_quarter_dose_delta":
                      r["same_polarity_half_dose_delta"]} for r in rows]):
            with self.assertRaises(ValueError):
                d.classify(bad)
        self.assertEqual(d.classify(REPORT["contrasts"]), REPORT["classification"])

    def test_fixed_reference_obstructions_and_retained_quarter_comparators(self):
        quarter_rows = {(r["control"], r["polarity"], r["left_id"], r["right_id"]): r
                        for r in SUMMARIES[3]["contrasts"]}
        self.assertEqual(len(quarter_rows), 36)
        ...
        for row, obstruction in zip(REPORT["contrasts"],
            ...
            retained = quarter_rows[row["control"], row["polarity"],
                                    row["left_id"], row["right_id"]]
            self.assertEqual(Fraction(row["predicted_delta"]),
                             Fraction(retained["predicted_delta"]) / 2)
            self.assertEqual(Fraction(row["same_polarity_quarter_dose_delta"]),
                             Fraction(retained["observed_delta"]))
            for dose in ("half", "full"):
                key = "same_polarity_" + dose + "_dose_delta"
                self.assertEqual(Fraction(row[key]), Fraction(retained[key]))
            self.assertIs(row["dose_comparable"], True)
            self.assertEqual(row["quarter_receipt_pair"], [row["left_id"], row["right_id"]])
            self.assertEqual(row["quarter_comparator_derivation"],
                             "retained_ordered_pair")
            self.assertEqual(row["reference_movement_identity"],
                             "fixed_reference_subtraction_cancels")
    ...
    def test_one_stdout_json_unknown_and_closed_non_admission_flags(self):
        ...
        for key, value in d.quarter.FLAGS.items():
        ...
        for key in ("candidate_admitted", "strict_FP16_state_claim", "new_token_claim",
                    ...
                    "closed_quarter_vector_intervention_replay", "full_dose_recomputation",
                    "half_dose_recomputation", "quarter_dose_recomputation", "dose_search"):
        ...
        self.assertEqual(len(REPORT["closed_results"]), 6)
