"""Frozen L16 lineage and L17 surface tests; native execution is always mocked."""
...
from ace3.model.candidates import diagnose_q24_s16_l17_from_l16_coordinate62_suffix_v1 as d


EXPECTED_TESTS = 32


class L17SuffixPlanTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.reports = json.loads((d.SELECTED / "actual_L16_gates.json").read_text())
    ...
    def test_only_l17_p0_and_disclosed_interface(self):
        for layer, position in ((True, 0), (16, 0), (18, 0), (17, 1)):
    ...
    def test_exact_thirteen_planned_controls_and_zero_check_dispatch(self):
        ...
        self.assertEqual(bounds["planned_native_L17_P0_invocations"], 13)
        self.assertEqual(bounds["layers"], [17])
        ...
        for key in ("native_layer_invocations", "native_L0_L16_invocations", "rtl_invocations"):
    ...
    def test_bounds_reject_expansion_replay_and_output_relaxation(self):
        for section in ("planned_bounds", "execution_bounds"):
            for key, value in (("layers", [16, 17]), ("native_L0_L16_invocations", 1),
    ...
    def test_l16_passes_do_not_erase_historical_failure(self):
        d.check_result(self.result)
        d.check_rows(self.rows)
        self.assertEqual(self.result["retained_status"], "FAIL")
        self.assertEqual(self.result["all_L16_gate_passing_controls"], list(d.CONTROLS))
        self.assertTrue(all(row["all_L16_gates_pass"] for row in self.rows))
    ...
    def test_result_rejects_claim_count_and_lineage_changes(self):
        for key, value in (("candidate_admitted", True), ("policy_adopted", True),
                           ...
                           ("all_L16_gate_passing_controls", list(d.parent.PASSING)),
    ...
    def test_rows_reject_kv_state_and_admission_changes(self):
        for key, value in (("prior_layer_kv_consumed", True), ("candidate_admitted", True),
                           ("L16_source_operand_state_KV_RTZ_checks", "FAIL"),
                           ("prior_kv", "L16 KV")):
            rows = deepcopy(self.rows)
            rows[0][key] = value
            with self.subTest(key=key), self.assertRaises(ValueError):
    ...
    def test_all_thirteen_parents_are_complete_independent_iz_h_copies(self):
        for label in d.CONTROLS:
            with self.subTest(control=label), \
                    d.np.load(d.SELECTED / (label + "_L16.npz"), allow_pickle=False) as arrays:
    ...
    def test_parent_rejects_fp16_only_and_spliced_state(self):
        with d.np.load(d.SELECTED / "actual_L16.npz", allow_pickle=False) as saved:
    ...
    def test_source_pins_reject_changed_l16_candidate_contract_or_tests(self):
        original = d.record
    ...
    def test_independent_l16_round_two_review_required(self):
        ...
        for key, value in (("producer_role", "engineer"), ("mission_id", "another-task"),
                           ("round", 1), ("review", {"status": "blocked"})):
            with self.subTest(key=key), self.assertRaises(ValueError):
                d.check_parent_review({**review, key: value})

    def test_reference_follows_original_input_l17_chain(self):
        self.assertIs(d.check_reference(self.extension), self.extension["layers"]["17"])

    def test_reference_rejects_reanchoring_kv_and_substitution(self):
        for layer, key in (("17", "input_binary64"), ("17", "input_fp16"), ("17", "prior_kv"),
        ...
        for key in ("binary64", "fp16"):
            extension = deepcopy(self.extension)
            extension["layers"]["17"][key]["sha256"] = "0" * 64
            with self.subTest(key=key), self.assertRaises(ValueError):
                d.check_reference(extension)
    ...
    def test_cli_check_only_calls_validation(self):
        with patch.object(d, "validate", return_value={"L17_status": "NOT_EXECUTED"}) as check, \
                patch("sys.stdout", new_callable=io.StringIO) as output:
            self.assertEqual(d.main(["--check"]), 0)
        check.assert_called_once_with()
        self.assertEqual(json.loads(output.getvalue()), {"L17_status": "NOT_EXECUTED"})
    ...
    def test_execution_interface_preserves_bounds_and_non_admission(self):
        ...
        self.assertEqual(bounds["native_L17_P0_invocations"], 13)
    ...
    def controls(self):
        parents = {}
        for label in d.CONTROLS:
            with d.np.load(d.SELECTED / (label + "_L16.npz"), allow_pickle=False) as arrays:
