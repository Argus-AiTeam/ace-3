...
struct Harness {
    ...
    explicit Harness(const std::string& dir) {
        ...
        const auto manifest = lines(dir + "/boundary_manifest.json");
        std::string manifest_text; for (const auto& line : manifest) manifest_text += line;
        if (input_lines.size() != 1792 || trace_lines.size() != 46676 ||
            final_lines.size() != 1792 || rope_lines.size() != 64 ||
            manifest_text.find("\"trace_records\": 46676") == std::string::npos ||
            manifest_text.find("\"final_records\": 1792") == std::string::npos)
            throw std::runtime_error("decoder vector manifest or count mismatch");
        ...
        for (const auto& line : trace_lines)
            trace.push_back({static_cast<unsigned>(std::stoul(line.substr(0,2),nullptr,16)),
        ...
        for (const auto& line : final_lines)
            final.push_back({static_cast<unsigned>(std::stoul(line.substr(0,2),nullptr,16)),
                             static_cast<unsigned>(std::stoul(line.substr(2,4),nullptr,16)),
        ...
        norm1 = hex_file<uint16_t>(tensor(dir, "layer0_input_layernorm_weight.fp16le.bin"));
        norm2 = hex_file<uint16_t>(tensor(dir, "layer0_post_attention_layernorm_weight.fp16le.bin"));
        p[0]=projection(dir,"layer0_self_attn_q_proj",true);
        p[1]=projection(dir,"layer0_self_attn_k_proj",true);
        p[2]=projection(dir,"layer0_self_attn_v_proj",true);
        p[3]=projection(dir,"layer0_self_attn_o_proj",false);
        p[4]=projection(dir,"layer0_mlp_gate_proj",false);
        p[5]=projection(dir,"layer0_mlp_up_proj",false);
        p[6]=projection(dir,"layer0_mlp_down_proj",false);
    ...
    void observe() {
        ...
        if (layer0_trace_capture_accept(
            ...
            if (trace_count>=trace.size() || trace[trace_count].token!=done_count ||
                trace[trace_count].position!=top.trace_position_o || trace[trace_count].stage!=top.trace_stage_o ||
                trace[trace_count].index!=top.trace_index_o || trace[trace_count].value!=top.trace_f16_o)
        ...
        if (layer0_final_capture_accept(
                ...
                top.trace_ready_i)) {
            if (final_count>=final.size() || final[final_count].token!=done_count ||
                final[final_count].index!=top.final_index_o || final[final_count].value!=top.final_f16_o ||
                top.final_last_o != (top.final_index_o==895))
...
int main(int argc, char** argv) {
    try {
        ...
        if(dir.empty()) throw std::runtime_error("usage: --vector-dir PATH");
        if(!preload_timeout_cycles || !start_timeout_cycles)
            throw std::runtime_error("timeout cycles must be nonzero");
        Harness h(dir); cycles=stalls=failures=trace_count=final_count=done_count=0; checking=false; h.idle(); h.reset();
        ...
        h.load(1,0); h.load(2,0); h.load(0,0); h.start(0,0,0); h.finish_unchecked();
        h.load(0,1); h.start(1,0,1); h.clear_after_work();
        ...
        h.top.final();
        if(failures) { std::cerr<<"DECODER_LAYER0_TOKEN_ENGINE_VERILATOR_FAIL failures="<<failures<<"\n"; return 1; }
        std::cout<<"DECODER_LAYER0_TOKEN_ENGINE_VERILATOR_PASS trace_count="<<trace_count
    ...
    } catch(const std::exception& e) { std::cerr<<"DECODER_LAYER0_TOKEN_ENGINE_VERILATOR_FAIL "<<e.what()<<"\n"; return 2; }
}
