...
from validate_selected_token_position2_traversal import (  # noqa: E402
    ...
    materialize_runtime_vector_contract,
...
class SelectedTokenPosition2ValidationTests(unittest.TestCase):
    ...
    def test_extracted_verilator_harness_enforces_runtime_vector_contract(
        self,
    ) -> None:
        build_parent = ROOT / "build"
        build_parent.mkdir(exist_ok=True)
        with tempfile.TemporaryDirectory(
            prefix="position2-interface-regression-",
            dir=build_parent,
        ) as directory:
            root = Path(directory)
            ...
            with tarfile.open(archive, "w") as stream:
                for relative in sorted(set(CONSUMED_SOURCE_PATHS.values())):
            ...
            compile_result = subprocess.run(
                [
                    ...
                    f"MODEL24_RTL_CASCADE_DIR={external_build}",
                ],
                check=False,
                capture_output=True,
                text=True,
            ...
            vector_dir = root / "position000/vectors"
            hidden_bits = np.zeros(896, dtype="<u2")
            with safe_open(
                ...
                oracle_values = layer_oracle_values(checkpoint, 0, vectors)
            ...
            runtime_records = materialize_runtime_vector_contract(
                0,
                0,
                final,
                trace,
            ...
            self.assertEqual(
                set(runtime_records),
                {"trace", "final_hidden", "boundary_manifest"},
            )

            raw_dir = root / "positive/raw"
            ...
            command = [
                ...
                "0",
                "--state-out",
                str(root / "positive/position001.state"),
                "--fail-after-raw",
            ]
            ...
            self.assertEqual(positive.returncode, 2)
            self.assertIn("injected failure after raw trace", positive.stderr)
            self.assertNotIn("cannot open", positive.stderr)
            self.assertNotIn("DECODER_CONTROLLER_MISMATCH", positive.stderr)
            self.assertEqual(
                len((raw_dir / "trace.hex").read_text(encoding="ascii").splitlines()),
                1,
            )

            (vector_dir / "trace.hex").unlink()
            negative_raw = root / "negative/raw"
            negative_raw.mkdir(parents=True)
            negative_command = command.copy()
            negative_command[negative_command.index(str(raw_dir))] = str(negative_raw)
            ...
            self.assertIn(
                f"cannot open {vector_dir}/trace.hex",
                negative.stderr,
            )
            self.assertNotIn("injected failure after raw trace", negative.stderr)
