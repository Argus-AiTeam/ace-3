...
def write_comparison_gate_fixture(
    root: Path,
    produced: np.ndarray,
    ...
    raw_dir = layer_dir / "raw"
    raw_dir.mkdir(parents=True)
    final_payload = raw_hidden_payload(produced)
    ...
    (raw_dir / "final.hex").write_bytes(final_payload)
    (raw_dir / "comparison.json").write_bytes(comparison_payload)
    record: dict[str, object] = {
        "output_raw_sha256": sha256(final_payload),
        "comparison": comparison,
        "artifacts": {
            "final.hex": artifact(final_payload),
            "comparison.json": artifact(comparison_payload),
        },
    }
    return layer_dir, record
...
class ControllerModel24RtlCascadeTests(unittest.TestCase):
    ...
    def test_existing_layer_without_record_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            binding, record, consumed, sources = write_completed_layer_fixture(root)
            (root / "layer07" / "record.json").unlink()
            with patch(
                "controller_model24_rtl_cascade."
                "materialize_indexed_decoder_vectors",
            ) as regenerate, self.assertRaisesRegex(
                RtlCascadeError,
                "record missing from existing layer directory",
            ):
                validate_completed_layer(
                    7,
                    root / "layer07",
    ...
    def test_missing_comparison_stops_before_oracle_regeneration(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            binding, record, consumed, sources = write_completed_layer_fixture(root)
            (root / "layer07" / "raw" / "comparison.json").unlink()
            with patch(
                "controller_model24_rtl_cascade."
                "materialize_indexed_decoder_vectors",
            ) as regenerate, self.assertRaisesRegex(
    ...
    def test_fresh_comparison_authorizes_handoff(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            produced = fp16_bits(np.ones((2, 896), dtype=np.float16))
            reference = np.ones((2, 896), dtype=np.float64)
            layer_dir, record = write_comparison_gate_fixture(
                Path(temporary),
                produced,
                reference,
                reference,
            ...
            handoff_path, handoff_sha256 = comparison_gated_handoff(
                7,
                layer_dir,
                record,
                reference,
            ...
            self.assertEqual(handoff_path, layer_dir / "raw" / "final.hex")
            self.assertEqual(handoff_sha256, record["output_raw_sha256"])

    def test_stale_comparison_cannot_authorize_next_handoff(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            produced = fp16_bits(np.ones((2, 896), dtype=np.float16))
            retained_reference = np.ones((2, 896), dtype=np.float64)
            current_reference = retained_reference.copy()
            current_reference[1, 23] = 1.0625
            layer_dir, record = write_comparison_gate_fixture(
                Path(temporary),
                produced,
                retained_reference,
            ...
            with self.assertRaisesRegex(
                RtlCascadeError,
                "comparison artifact is stale or mismatched",
            ):
                comparison_gated_handoff(
                    7,
                    layer_dir,
            ...
            self.assertFalse((Path(temporary) / "layer08").exists())

    def test_mismatched_comparison_artifact_cannot_authorize_handoff(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            produced = fp16_bits(np.ones((2, 896), dtype=np.float16))
            reference = np.ones((2, 896), dtype=np.float64)
            layer_dir, record = write_comparison_gate_fixture(
            ...
            comparison_path = layer_dir / "raw" / "comparison.json"
    ...
    def test_true_comparison_failure_cannot_authorize_next_handoff(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            reference = np.ones((2, 896), dtype=np.float64)
            produced = fp16_bits(reference)
    ...
    def test_layer_reference_restarts_from_authenticated_fp16_handoff(self) -> None:
        ...
        with patch(
            ...
            side_effect=lambda _layer, _tensors, activation, **_: activation,
        ) as reference_layer:
            result = same_handoff_reference_layer(7, {}, handoff)

        reference_layer.assert_called_once()
        self.assertEqual(
            reference_layer.call_args.kwargs["reference_policy"],
