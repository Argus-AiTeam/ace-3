...
TIED_HEAD_ATTEMPT_DIR = (
    REPOSITORY_ROOT / "build" / "model24_tied_lm_head_topk_attempt002"
)
...
class TiedHeadTokenizerHostIntegrationTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.tokenizer_dir = ACCEPTED_HOST_TOKENIZER_DIR
        cls.document = create_tied_head_tokenizer_host_integration(
            TIED_HEAD_ATTEMPT_DIR,
            cls.tokenizer_dir,
        )

    def test_current_lineage_token_zero_is_appended_and_decoded(self) -> None:
        ...
        self.assertEqual(summary["attempt_id"], TIED_HEAD_INTEGRATION_ATTEMPT_ID)
        self.assertEqual(summary["selected_token_id"], TIED_HEAD_SELECTED_TOKEN_ID)
        self.assertEqual(
            self.document["selection"]["selected_logit_f16_bits"],
            TIED_HEAD_SELECTED_LOGIT_F16_BITS,
        )
        self.assertEqual(summary["top_k_entries"], 10)
        self.assertEqual(summary["resulting_token_history"], [9707, 1879, 0])
        self.assertEqual(summary["decoded_transcript_fragment"], "Hello world!")
        self.assertEqual(
            self.document["tokenizer_binding"]["seed_prompt"],
            {
        ...
        self.assertEqual(
            self.document["source_lineage"]["tied_head"]["review_status"],
            "PENDING_INDEPENDENT_REVIEW",
        )

    def test_required_drift_is_rejected(self) -> None:
        mutations = (
            (
                "tokenizer",
                lambda document: document["tokenizer_binding"][
                    "official_assets"
                ]["tokenizer"].__setitem__("sha256", "0" * 64),
                "tokenizer/chat-template binding drift",
            ),
            (
                "prompt_history",
                lambda document: document["host_result"][
                    "input_token_history"
                ].append(0),
            ...
            (
                "top_k_order",
                lambda document: document["selection"]["top_k"].__setitem__(
                    slice(0, 2),
                    list(reversed(document["selection"]["top_k"][:2])),
                ),
                "Top-K payload/order",
            ),
            (
            ...
            (
                "selected_logit",
                lambda document: document["selection"].__setitem__(
                    "selected_logit_f16_bits", 0x4C0F
                ),
                "selected-logit drift",
            ),
            (
                "predecessor",
                lambda document: document["source_lineage"]["final_rmsnorm"].__setitem__(
                    "seal_sha256", "0" * 64
                ),
                "predecessor-lineage drift",
            ),
        )
