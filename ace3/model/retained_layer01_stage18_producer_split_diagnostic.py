#!/usr/bin/env python3
"""Retained A-lineage layer01 accounting, without RTL or trajectory replay."""
import argparse
import hashlib
import json
import os
...
PARENT = ROOT / "build/retained_layer02_stage18_producer_split_01ce0c7e0c31_attempt001"
REVIEW = Path("/home/argustest/.argus-skill-ace3/projects/s-62150b05/handoffs/01ce0c7e0c31")
PACKAGE = ROOT / "build/token358_position3_layer1_preflight_attempt002/package"
ROOTS = {
    "frozen.json": "5228dd8970120638d964c2ac7bf012db192c4406fa277dac9f59061705579dd7",
    "result.json": "be747f1454b99b9fd5719a299c2492d4013193a9dff8f860257c9b19c589680a",
    "artifacts.json": "cf65928f2ae967783d9da08156420221ac136688537c9741c1261c19b2b405bc",
}
QUALIFICATION = (
    ...
    "Positions2/3 inherit that historical qualification. Supplemental layer00/01 "
    "roots and position3 execution receipts are explicitly content-bound, not "
    "promoted to original execution provenance. Stage16-to-down joins are "
    "source-derived ordered accesses, not captured read-bus observations."
)
...
def command(name, argv):
    ...
    write(name + ".status.json", dict(returncode=result.returncode,
                                      seconds=time.monotonic() - start))
    require(result.returncode == 0, name + " failed; see retained log")
...
def bootstrap(attempt):
    ...
    require(receipt["kind"] == "round_reviewed_handoff"
            ...
            and receipt["mission_id"] == "01ce0c7e0c31"
            and receipt["review"]["status"] == "done"
            and "layer01 stage18" in receipt["review"]["next_action"],
            "mission-scoped reviewed parent required")
    for name, digest in ROOTS.items():
        require(record(PARENT / name)["sha256"] == digest and digest in checkpoint,
                "reviewed parent identity changed: " + name)
    ...
    package_record = next(rec for rec in parent_freeze["authenticated_inputs"]
                          if rec["path"] == str(PACKAGE / "launch_package.json"))
    require(record(package_record["path"]) == package_record, "parent-bound layer01 package changed")
    package = json.loads(Path(package_record["path"]).read_text())
    dependencies = []
    for name in ("ace3_qwen2_rope_pair.sv", "ace3_attention_score_core.sv"):
        rec = next(rec for rec in package["sources"] if rec["path"].endswith("/" + name))
        require(record(rec["path"])["sha256"] == rec["sha256"],
                "historical decoder dependency changed: " + name)
        dependencies.append(Path(rec["path"]))
    if attempt == 2:
        failed = OUT.with_name("retained_layer01_stage18_producer_split_a947a562a897_attempt001")
        write("prior_attempt_diagnosis.json", dict(
            attempt=1, manifest=record(failed / "artifacts.json"),
            compile_log=record(failed / "public_contract_compile.log"),
            failure_taxonomy="public_contract_elaboration",
            root_cause_hypothesis="Parent operator-only snapshot omitted ace3_qwen2_rope_pair "
            "and ace3_attention_score_core required by the newly compiled full decoder top.",
            regression="Compile the exact unchanged decoder top with both historical hash-bound "
            "dependencies, then execute the complete original coordinate/oracle accounting.",
            semantic_measurement_executed=False, RTL_correctness_conclusion=None))
    sources = OUT / "sources"
    sources.mkdir()
    snapshots = []
    for path in sorted((PARENT / "sources").iterdir()) + dependencies + [Path(__file__).resolve()]:
    ...
    command("measurement", [sys.executable, "-B", str(sources / Path(__file__).name),
                            "--attempt", str(attempt), "--measure"])
    seal()
    result = json.loads((OUT / "result.json").read_text())
    print(json.dumps({key: result[key] for key in (
        "status", "decision", "own_input_local_discrepancies", "summaries",
        "upstream_boundaries", "phases_seconds")}, sort_keys=True))
    print(json.dumps({name: record(OUT / name) for name in ROOTS}, sort_keys=True))
...
def measure():
    ...
    def read_supplemental(path):
        path = Path(path).resolve()
        fresh = str(path) not in inputs.bindings
        data = inputs.read(path, root=fresh)
        if fresh:
            supplemental.append(inputs.records[str(path)])
        return data
    ...
    load(PACKAGE / "frozen.json")
    pre = load(PACKAGE / "launch_package.json")
    kv = pre["kv_parent"]
    ...
    require(pre["layer_index"] == old["layer_index"] == kv["layer_index"] == 1
            ...
            "retained A-lineage contract/state mismatch")
    for rec in (kv["live_binary"], kv["state"], *kv["position_states"]):
        inputs.read(rec["path"])
    execution = PACKAGE.parent / "future_execution/position003"
    terminal = load(execution / "simulation.log.exit.json")
    invocation = load(terminal["command"]["path"])
    inputs.read(terminal["log"]["path"])
    require(terminal["exit_code"] == 0 and invocation["argv"] == pre["future_argv"],
            "retained execution command/terminal mismatch")
    live_binary = record(invocation["argv"][0])
    require(live_binary["sha256"] == kv["live_binary"]["sha256"],
            "position3 restored binary differs from historical binary")
    read_supplemental(invocation["argv"][0])
    actual = {p: boundary.decode_trace(inputs.read(rec["path"]), p)
              for p, rec in enumerate(kv["actual_kv_traces"])}
    actual[3] = boundary.decode_trace(read_supplemental(execution / "raw/trace.hex"), 3)
    incoming = [from_hex(old["positions"][p]["vectors"]["input"] if p < 3
                         else pre["input_hidden"], True) for p in range(4)]
    layer0 = load(Path(kv["layer_result"]["path"]).parents[1] / "layer00/result.json")
    require(layer0["layer_index"] == 0, "layer00 producer layer")
    for p in range(3):
        require(layer0["positions"][p]["position"] == p
                and from_hex(layer0["positions"][p]["output"], True) == incoming[p],
                "actual layer00-stage18 to layer01-input join")
    layer0_position3 = load(pre["layer0_result"]["path"])
    require(layer0_position3["layer"] == 0 and layer0_position3["position"] == 3
            and from_hex(layer0_position3["output_hidden"], True) == incoming[3],
            "position3 actual layer00-to-layer01 join")
    stages = (11, 12, 14, 15, 16, 17, 18)
    reference = {p: {} for p in range(4)}
    reference_incoming = {}
    history = load(pre["independent_parent"]["path"])
    reference_root = Path(pre["independent_parent"]["path"]).parent
    early = load(reference_root / "layer01_generation0.json")
    upstream_early = load(reference_root / "layer00_generation0.json")
    upstream_history = load(reference_root / "layer00_generation1.json")
    for first, later, layer in ((early, history, 1), (upstream_early, upstream_history, 0)):
        for obj, positions, tokens in ((first, [0, 1], boundary.HISTORY[:2]),
                                       (later, [2], boundary.HISTORY[:3])):
            require(obj["layer"] == layer and obj["history"] == tokens
                    and obj["positions"] == positions, "reference layer/history/position mismatch")
            for rec in obj["stages"]:
                stage = int(Path(rec["path"]).stem[5:])
                p = int(Path(rec["path"]).parent.name[8:])
                if layer == 1 and stage in stages:
                    reference[p][stage] = from_array(rec)
                elif layer == 0 and stage == 18:
                    reference_incoming[p] = from_array(rec)
        require(first["checkpoint_tensor_hashes"] == later["checkpoint_tensor_hashes"],
                "historical checkpoint tensor identity mismatch")
        for name in ("k", "v"):
    ...
    for stage in stages:
        reference[3][stage] = from_hex(pre["independent_expected_stages"][str(stage)])
    reference_incoming[3] = from_hex(pre["independent_layer0_hidden"])
    parent_rows = {p: inputs.load(PARENT / f"position{p}_incoming_coordinates.json") for p in range(4)}
    for p in range(4):
        require(len(parent_rows[p]) == len(actual[p][18]) == 896, "parent join geometry")
        for i, row in enumerate(parent_rows[p]):
            require(row["position"] == p and row["index"] == i
                    and int(row["actual"], 16) == actual[p][18][i]
                    and int(row["reference"], 16) == reference[p][18][i],
                    "reviewed parent layer01-stage18 actual/reference join")
    parent_output = next(rec for rec in parent_freeze["authenticated_inputs"]
                         if rec["path"] == str(execution / "raw/final.hex"))
    require(from_hex(parent_output, True) == actual[3][18], "position3 trace/final join")
    tensors, tensor_records = {}, []
    for rec in pre["vectors"]["tensors"]:
        meta = rec["checkpoint_tensor"]
        if not meta["name"].startswith("model.layers.1.mlp.down_proj."):
            continue
    ...
    tops = ("ace3_decoder_layer0_token_engine", "ace3_awq_w4a16_projection_engine",
            "ace3_fp16_residual_add_core", "ace3_fp16_silu_gate_core")
    for top in tops:
        source = next(rec for rec in pre["sources"] if rec["path"].endswith("/" + top + ".sv"))
        require(record(OUT / "sources" / (top + ".sv"))["sha256"] == source["sha256"],
                "historical public source mismatch: " + top)
    parameters = {
        "ace3_decoder_layer0_token_engine.LAYER_INDEX": 1,
        "ace3_decoder_layer0_token_engine.ACCURATE_SILU": 1,
    ...
    require(tool is not None, "host iverilog unavailable; inspect declared containers before claiming unavailable")
    ...
    write("frozen.json", dict(
        ...
        positions=[0, 1, 2, 3], layer=1, authenticated_inputs=list(inputs.records.values()),
        ...
        public_parameters=parameters, public_port_contract=pre["public_contract"],
        ...
        numerical_policy="Native G128 asymmetric GEMM lanes 0,4,1,5,2,6,3,7; no qzero +1; "
        "FP16 scales/activations/KV. Exact Q48 down accumulation with signed 96-bit group "
        "and 102-bit cross-group bounds then FP16 RNE. Residual exact Q24 sum then FP16 RNE. "
        "SiLU polynomial Q24 sigmoid; Q72 product -> Q24 RNE -> FP16 RNE; signed underflow "
        "zero follows frozen operator helpers. Separate Decimal80 mathematical SiLU and "
        "binary64 SiLU recurrence do not replace the retained independent reference.",
        accounting="Both residual substitution orders and both gate/up orders. Each down term "
        "is coefficient_q24[input,output] times the position-specific stage16 Q24 component. "
        "All terms retained without pruning, with output and G128 exact Q48 totals.",
        validation="One semantic execution; existing helper regressions and independent "
        "all-coordinate residual, SiLU and two-input down integer oracles.",
    ...
    def split(chain, names):
        ...
        require(sum(result.values()) == units(chain[0]) - units(chain[-1]), "Q24 closure")
        return result
    ...
    def save_rows(p, name, rows):
        ...
        summaries[f"p{p}_{name}"] = dict(
            coordinates=len(rows), bit_differences=sum(r["actual"] != r["reference"] for r in rows),
    ...
    for p in range(4):
        ...
        require(len(h) == len(rh) == 896, "incoming geometry")
        upstream[p] = {}
        for name, left, right in (("layer00_stage18_incoming_hidden", h, rh),
                                 ("layer01_stage11_O", a[11], r[11]),
                                 ("layer01_stage14_gate", a[14], r[14]),
                                 ("layer01_stage15_up", a[15], r[15])):
            upstream[p][name] = dict(coordinates=len(left),
                bit_differences=sum(x != y for x, y in zip(left, right, strict=True)),
        ...
        write(f"position{p}_incoming_coordinates.json", [
            dict(position=p, index=i, actual=f"{h[i]:04x}", reference=f"{rh[i]:04x}",
                 ...
                 actual_layer00_output_join=True, inherited_historical_provenance_qualified=True)
            for i in range(896)])
        for stage, la, lr, ra, rr, names in (
            ...
            require(all(len(v) == 896 for v in (la, lr, ra, rr, a[stage], r[stage])), "residual geometry")
            rows = []
            for i in range(896):
                cases = {}
                ...
                row.update(operand_names=list(names), left_actual=f"{la[i]:04x}",
                           left_reference=f"{lr[i]:04x}", right_actual=f"{ra[i]:04x}",
                           right_reference=f"{rr[i]:04x}",
    ...
    for p in range(4):
        ...
        for i in range(4864):
            ...
            require(not (invalid or saturated) and reconstructed["bits"] == oracle,
                    "independent own-input SiLU oracle")
            ideal = {}
        ...
        for i in range(896):
            ...
            for label, activation in (("A", a[16]), ("R", r[16])):
                ...
                require(not (oracle[2] or oracle[3]) and oracle[0] == totals[label][i]
                        ...
                        and -(1 << 101) <= oracle[0] < (1 << 101), "independent down oracle")
            ...
            row.update(actual_sum_q48=totals["A"][i], reference_sum_q48=totals["R"][i],
                       ...
                       input_coordinates=4864, coefficient_column=i, source_derived_not_observed_bus=True)
            for key, values in propagated.items():
                components = {name: values[name][i] for name in values}
                require(sum(components.values()) == totals["A"][i]-totals["R"][i], "down input closure")
                components.update(
    ...
    inherited = sum(v["layer00_stage18_incoming_hidden"]["bit_differences"] for v in upstream.values())
    decision = (
        "Inherited layer00 stage18 output drift is already present at the layer01 input, "
        "the earliest supported boundary in this scope, not a unique historical root cause."
        if inherited else "No inherited layer00 bit drift; inspect layer01 O and gate/up producer boundaries."
    )
    decision += (" Layer01 residual, SiLU and down reconstruct exactly on their own inputs."
                 if not discrepancies else " Local reconstruction discrepancies remain explicitly accounted.")
    write("result.json", dict(
        ...
        next_exact_upstream_boundary="Batch layer00 stage18 residual/down and its residual/MLP "
        "producers against the authenticated token-root inputs. Layer01 O and gate/up are also "
        "contributors, not uniquely localized defects. No measured downstream benefit supports "
        "a repair candidate from this decomposition alone.",
...
if __name__ == "__main__":
    ...
    require(args.attempt > 0, "positive fresh attempt number required")
    OUT = ROOT / f"build/retained_layer01_stage18_producer_split_a947a562a897_attempt{args.attempt:03d}"
    if args.measure:
        require(Path(__file__).resolve().parent == OUT / "sources", "measurement must use frozen sources")
        measure()
    else:
        OUT.mkdir()
        with (OUT / "launch.command.sh").open("x", encoding="ascii") as stream:
            stream.write(shlex.join([sys.executable, "-B", *sys.argv]) + "\n")
        try:
            bootstrap(args.attempt)
            ...
            write("failure.json", dict(
                ...
                regression="Fresh attempt must close all coordinates against independent oracles.",
