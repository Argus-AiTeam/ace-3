"""Exact thirty-second terminal-vector polarities through a non-admission CPU suffix."""
...
from ace3.model.candidates import diagnose_q24_s16_terminal_remainder_sixteenth_vector_dose_contrast_v1 as sixteenth


eighth, quarter, half, reverse, vector, complement, closed, bridge, base, parent, hotspot = (
    sixteenth.eighth, sixteenth.quarter, sixteenth.half, sixteenth.reverse,
    sixteenth.vector, sixteenth.complement, sixteenth.closed, sixteenth.bridge,
    sixteenth.base, sixteenth.parent, sixteenth.hotspot)
require, same, ROOT = base.require, base.same, base.ROOT
NAME = "diagnose_q24_s16_terminal_remainder_thirtysecond_vector_dose_contrast_v1"
...
COORDINATES, POLARITIES, DOSE = tuple(range(896)), ("forward", "reverse"), Fraction(1, 32)
EXPECTED_TESTS = 14
SIXTEENTH_CHAIN = (
    sixteenth, "453240259c8d",
    "0e2a2c3fedfb9f8e1df9a9aec5da17a20f0b570418108d82dab66f91b1e47a8b",
    "0edd081c39b97770eaaeac69d2e454a81c0e036f9fa896323ca3ea61753e70e7",
    "2c073e860319127a8df9478825adc5ddfd9a9fccd5539a54afe731e81ada6b53",
)
SIXTEENTH_RECEIPT = {
    "path": "/home/argustest/.argus-skill-ace3/copilot-home/session-state/"
            "d214d4cc-fbc5-4272-b906-c012f40c7f6d/events.jsonl",
    "sha256": "bc581f95e342d490e900b2827cfdb5047973849c2f7bdf7ea17d715d5f3a6e8f",
}
SIXTEENTH_CALL = "call_ZW5adPPhqLooKOOTCxvrEPop"
RECEIPTS = (*sixteenth.RECEIPTS, SIXTEENTH_RECEIPT)
DOSE_KEYS = tuple(f"same_polarity_{dose}_dose_delta"
                  for dose in ("sixteenth", "eighth", "quarter", "half", "full"))
CONTRAST_FIELDS = (
...
PREREGISTRATION = {
    ...
    "operand": "Freeze delta = original_FP16_stage18 - original_binary64_terminal. "
               "Working[i] = native_RNE_FP16(actual_FP16_stage18[i] +/- delta[i]/32), "
               "minus forward, plus reverse. Exact division by 32 precedes one conversion; "
               "never average rounded sixteenth/eighth/quarter/half/full operands.",
    "prediction": "All 72 control/polarity/ordered-pair/original-reference contrasts move "
                  ...
                  "direction, with 0 < abs(thirty-second movement) < abs(same-polarity "
                  "sixteenth movement), and below same-polarity eighth/quarter/half/full "
                  "when comparable.",
    "comparison": "Authenticate reviewed source/test/review pins and closed full/reverse/"
                  "half/quarter/eighth/sixteenth receipts with identical source/token/"
                  "operand/Q24/state/KV/lineage/reference/weight identities. The sixteenth "
                  "receipt retains all 36 binary64 control/polarity/ordered-pair contrasts "
                  "as contrast_fields plus contrast_values. Require the exact ordered "
                  "census, predictions, older dose comparators and original-reference "
                  "margin closure. Fixed-reference subtraction cancels for either original "
                  "branch. Missing, ambiguous or incompatible comparators are UNKNOWN, "
                  "never omitted passing rows. No coordinate gate census or selected "
                  "union is reconstructed.",
    "supported": "Conversion-resilient sub-sixteenth local terminal-boundary dose "
                 "sensitivity; a smaller fixed-dose successor is justified.",
    "rejected": "Below-sixteenth FP16 conversion-threshold/asymmetry/cancellation follow-up; "
    ...
    "boundary": sixteenth.PREREGISTRATION["boundary"].replace(
        "quarter/eighth/middle-pair", "quarter/eighth/sixteenth/middle-pair"),
}
FLAGS = {**sixteenth.FLAGS, "closed_sixteenth_vector_intervention_replay": False,
         "sixteenth_dose_recomputation": False}


def authenticate_chain():
    pins = sixteenth.authenticate_chain()
    module, mission, source, test, review = SIXTEENTH_CHAIN
    ...
    same((record["kind"], record["mission_id"], record["producer_role"],
         ...
         "closed sixteenth independent review gate failed")
    ...
    same((latest["kind"], latest["handoff"]["path"]),
         ("handoff_ref", str(review_path)), "closed sixteenth review lineage changed")
    pins.append(parent.record(latest_path))
    return pins


def retained_doses():
    previous = sixteenth.retained_doses()
    events = [json.loads(line) for line in base.read_bound(SIXTEENTH_RECEIPT).splitlines()]
    matches = [e["data"] for e in events if e["type"] == "tool.execution_complete"
               and e["data"]["toolCallId"] == SIXTEENTH_CALL]
    same(len(matches), 1, "sixteenth dose receipt missing or duplicated")
    require(matches[0]["success"] is True, "sixteenth dose receipt execution failed")
    text = matches[0]["result"]["content"]
    marker = '{"changed_coordinate_counts":'
    same(text.count(marker), 1, "sixteenth dose summary missing or duplicated")
...
def no_closed_replay(audit):
    def refuse(*args, **kwargs):
        ...
        raise RuntimeError("closed sixteenth-dose intervention replay forbidden")

    with sixteenth.no_closed_replay(audit), ExitStack() as stack:
        for name in ("check", "run_tests", "prepare"):
            stack.enter_context(patch.object(sixteenth, name, refuse))
        yield
...
def selection_gate():
    same((IDS, PAIRS, BRANCHES, COORDINATES, POLARITIES),
         ((319, 34319), ((319, 34319), (34319, 319)), ("fp16", "binary64"),
          tuple(range(896)), ("forward", "reverse")), "thirty-second-dose selection changed")
    require(type(DOSE) is Fraction and DOSE == Fraction(1, 32), "thirty-second dose changed")


def prepare(words, delta, polarity):
    selection_gate()
    require(polarity in POLARITIES, "invalid thirty-second-dose polarity")
    ...
    for i in COORDINATES:
        ...
        require(not saturated, "thirty-second-dose operand saturated")
        working[i] = bits
...
def bind_doses(evidence, forward, backward, retained_half, retained_quarter,
               retained_eighth, retained_sixteenth):
    selection_gate()
    delta, predictions = sixteenth.bind_doses(
        evidence, forward, backward, retained_half, retained_quarter, retained_eighth)
    same([retained_sixteenth[k] for k in (
        ...
        ["supported", 0, 1, 72, 72, 72, "UNKNOWN"], "closed sixteenth outcome/census changed")
    same([retained_sixteenth["tests"][k] for k in ("executed", "errors", "failures", "skipped")],
         [14, 0, 0, 0], "closed sixteenth test gate failed")
    same(retained_sixteenth["tests"]["compiled"],
         [parent.record(sixteenth.SOURCE), parent.record(sixteenth.TEST)],
         "closed sixteenth source/test binding changed")
    same(retained_sixteenth["dispatch_and_write_audit"],
         {"forbidden_calls": 0, "final_rmsnorm_invocations": 27,
          "selected_row_head_invocations": 27}, "closed sixteenth dispatch changed")
    same(retained_sixteenth["protected_input_identity"], closed.digest(evidence),
         "sixteenth comparison source/token/operand/state/KV/lineage/reference drift")
    same(retained_sixteenth["changed_coordinate_counts"],
         {p: {c: 827 for c in parent.CONTROLS} for p in POLARITIES},
         "closed sixteenth operand census changed")
    same(retained_sixteenth["contrast_fields"], list(CONTRAST_FIELDS),
         "closed sixteenth contrast schema changed")
    values = retained_sixteenth["contrast_values"]
    require(isinstance(values, list) and len(values) == 36
            and all(isinstance(v, list) and len(v) == len(CONTRAST_FIELDS) for v in values),
            "closed sixteenth contrast shape changed")
    ...
    same([(r["control"], r["polarity"], r["left_id"], r["right_id"], r["branch"])
         ...
         [(c, p, l, r, "binary64") for c in parent.CONTROLS for p in POLARITIES
          for l, r in PAIRS], "closed sixteenth ordered contrast census changed")
    comparisons = {}
    for row in rows:
        ...
        for field in ("predicted_delta", *DOSE_KEYS[1:]):
            same(row[field], prediction[field], "closed sixteenth prediction/dose drift")
        movement = Fraction(row["observed_delta"])
        require(Fraction(row["predicted_delta"]) * movement > 0
                and all(0 < abs(movement) < abs(Fraction(row[k])) for k in DOSE_KEYS[1:]),
                "closed sixteenth strict dose outcome changed")
        ...
        same(row["retained_margin_change"], str(retained),
             "closed sixteenth original-reference margin drift")
        same(row["intervened_margin_change"], str(retained + movement),
             "closed sixteenth margin movement closure failed")
        comparisons[control, left, right, polarity] = {
            **prediction,
            "predicted_delta": str(Fraction(prediction["predicted_delta"]) / 2),
            DOSE_KEYS[0]: str(movement),
            "sixteenth_receipt_branch": "binary64",
            "sixteenth_comparator_derivation": "closed_ordered_pair_margin_movement",
        }
...
def classify(rows):
    ...
    same([(r["control"], r["polarity"], r["left_id"], r["right_id"], r["branch"])
         ...
         [(c, p, l, r, b) for c in parent.CONTROLS for p in POLARITIES
          for l, r in PAIRS for b in BRANCHES], "incomplete or reordered thirty-second census")
    supported = True
    for row in rows:
...
def run_tests(evidence, report, outputs, identity, delta, summaries):
    ...
    spec = importlib.util.spec_from_file_location("terminal_thirtysecond_dose_tests", TEST)
...
def check():
    ...
    origins = {**parent.source_context(), MODULE: parent.record(SOURCE),
               "terminal_thirtysecond_dose_tests": parent.record(TEST)}
    ...
    report = {
        ...
        "retained_dose_calls": [reverse.PREDICTION_CALL, half.REVERSE_CALL,
                               quarter.HALF_CALL, eighth.QUARTER_CALL,
                               sixteenth.EIGHTH_CALL, SIXTEENTH_CALL],
        ...
        "closed_results": [
            ...
            for index, chain in enumerate((
                ...
                sixteenth.EIGHTH_CHAIN, SIXTEENTH_CHAIN))],
    }
