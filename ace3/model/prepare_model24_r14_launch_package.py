#!/usr/bin/env python3
"""Construct and inertly validate a fresh Model24 r14 launch candidate."""
...
CONTRACT = REPOSITORY / "ace3/contracts/model24_r14_durable_launch_contract.json"
VALIDATOR = REPOSITORY / "ace3/model/validate_model24_r14_launch_package.py"
LIFECYCLE = REPOSITORY / "ace3/model/model24_r14_lifecycle.py"
R12_PACKAGE = Path(
...
def write_review(
    ...
    **changes: Any,
    ...
    review = {
        "schema_version": 1,
        "kind": "ace3_model24_r14_independent_l2_review",
...
def run_negative_tests(package: Path) -> dict[str, Any]:
    validator = load_module(package / "validate-package.py", "model24_r14_validator")
    manifest = validator.load_json(package / "package.json")
    cases: list[str] = []
    canonical = validator.expected_paths(package, manifest)

    with tempfile.TemporaryDirectory(prefix="ace3-model24-r14-inert-") as temporary:
        ...
        _expect_rejection(
            lambda: validator.validate_terminal_root_value(
                Path("/tmp/ace3-model24-r14-terminal-fixture")
            ),
        ...
        require(
            ...
            "inert fixture created a canonical r14 artifact",
        )
    ...
    require(
        ...
        "negative case order differs from r14 contract",
    )
    return {
        "status": "PASS",
        "kind": "ace3_model24_r14_inert_negative_tests",
...
"""
    ...
    task_id = f"ace3-model24-r14-inert-probe-{nonce}"
    ...
    submission = [
        ...
        task_id,
        "--description",
        "ACE-3 r14 inert submitter-root and child-cwd integration probe",
        "--mode",
        "direct",
        "--timeout",
        "30",
    ...
    evidence = {
        "schema_version": 1,
        "kind": "ace3_model24_r14_installed_direct_runner_probe",
        "status": "PASS",
        "task_id": task_id,
...
def run_controller_entry_integration(package: Path) -> dict[str, Any]:
    lifecycle = load_module(package / "lifecycle.py", "model24_r14_probe_lifecycle")
    ...
    try:
        controller = load_module(
            ...
            "model24_r14_inert_controller_entry",
        )
    finally:
        sys.path.pop(0)
    ...
    def invoke(simulation_dir: Path, payload_output_dir: Path) -> dict[str, str]:
        ...
        def inert_execute(
            ...
            *,
            ...
            return {
                "kind": "ace3_model24_r14_inert_controller_entry",
    ...
    cases: list[str] = []
    with tempfile.TemporaryDirectory(prefix="ace3-model24-r14-controller-entry-") as temporary:
        scratch = Path(temporary)
    ...
    return {
        "schema_version": 1,
        "kind": "ace3_model24_r14_inert_controller_entry_probe",
        "status": "PASS",
...
def prepare(nonce: str) -> dict[str, Any]:
    base = Path(f"/home/argustest/ace3-model24-r14-prep-20260829-{nonce}")
    package = base / "package"
    source = base / "source"
    validation = base / "validation"
    preparation = base / "preparation"
    review = Path(f"/home/argustest/ace3-model24-r14-review-20260829-{nonce}")
    authority = Path(f"/home/argustest/ace3-model24-r14-authority-20260829-{nonce}.json")
    consumed = Path(str(authority) + ".consumed")
    output = Path(f"/home/argustest/ace3-model24-r14-output-20260829-{nonce}")
    simulation_dir = output / "controller-simulation"
    payload_output_dir = output / "rtl-cascade"
    terminal_root = Path(
        f"/home/argustest/ace3-model24-r14-terminal-20260829-{nonce}"
    )
    task_id = f"ace3-model24-r14-42c895c-{nonce}"
    for path in (
        base,
        review,
        authority,
        ...
        require(not path.exists(), f"fresh r14 namespace already exists: {path}")
    ...
    r12_evidence = {
        "schema_version": 1,
        "kind": "ace3_model24_r14_preserved_r12_terminal_evidence",
    ...
    r13_evidence = {
        "schema_version": 1,
        "kind": "ace3_model24_r14_preserved_r13_failure_evidence",
        "nonce": R13_NONCE,
        "task_id": R13_TASK_ID,
        "original_paths": {
    ...
    source_manifest = {
        "schema_version": 1,
        "kind": "ace3_model24_r14_source_tree",
        "commit": COMMIT,
        "git_tree": TREE,
    ...
    submission_argv = [
        ...
        task_id,
        "--description",
        "ACE-3 r14 Model24 native AWQ cascade, independently accepted and Manager-authorized exactly once",
        "--mode",
        "direct",
        "--timeout",
        "18000",
    ...
    manifest = {
        "schema_version": 1,
        "kind": "ace3_model24_r14_durable_launch_package",
        ...
        "review_policy": {
            "preparer_identity": f"role:engineer/session:r14-{nonce}",
    ...
    validator = load_module(VALIDATOR, "repository_r14_validator")
    review_request = {
        "schema_version": 1,
        "kind": "ace3_model24_r14_independent_l2_review_request",
        "candidate": str(base),
        "nonce": nonce,
        "task_id": task_id,
    ...
    seal = {
        "schema_version": 1,
        "kind": "ace3_model24_r14_package_seal",
        "nonce": nonce,
        "task_id": task_id,
        "package_manifest_sha256": digest(package / "package.json"),
    ...
    validation_seal = {
        "schema_version": 1,
        "kind": "ace3_model24_r14_inert_validation_seal",
