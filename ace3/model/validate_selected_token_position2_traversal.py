...
def materialize_runtime_vector_contract(
    layer_index: int,
    position: int,
    final: Sequence[int],
    trace: Sequence[tuple[int, int, int, int]],
    ...
    trace_path = vector_dir / "trace.hex"
    final_path = vector_dir / "final.hex"
    trace_path.write_bytes(oracle_trace_payload(trace))
    final_path.write_bytes(hidden_payload(np.asarray(final, dtype="<u2")))
    manifest_path = vector_dir / "boundary_manifest.json"
    ...
    return {
        "trace": file_record(trace_path),
        "final_hidden": file_record(final_path),
        "boundary_manifest": file_record(manifest_path),
    }
...
def execute_exact_transaction(
    ...
    runtime_vectors = {
        ...
        **materialize_runtime_vector_contract(
            ...
            final,
            trace,
            Path(vectors["input"]["path"]).parent,
        ),
    }
