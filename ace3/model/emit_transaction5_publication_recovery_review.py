#!/usr/bin/env python3
"""Source-disjoint review of the transaction005 recovery adjudication."""
...
TRANSACTION5 = RUNTIME / "transactions/transaction-005"
...
def validate_with_production_completion_validator(
    ...
    transaction = load_json(TRANSACTION5_PACKAGE_MANIFEST)[
        "transaction_descriptor"
    ]
    spec = importlib.util.spec_from_file_location(
        "ace3_transaction5_review_production_control",
        PRODUCTION_CONTROL,
    )
    require(
    ...
    return {
        ...
        "transaction_descriptor": file_record(TRANSACTION5_PACKAGE_MANIFEST),
...
def validate_adjudication(document: Mapping[str, Any], authenticate_all: bool) -> None:
    ...
    require(
        ...
        and finding.get("generation6_publication_attempted") is False,
    ...
    require(
        ...
        == file_record(TRANSACTION5_PACKAGE_MANIFEST)
    ...
    require(
        ...
        "generation6 or transaction006-025 exists",
    )
    require(
        FUTURE.is_dir()
    ...
    inventory = document.get("frozen_transaction005", {}).get("inventory")
    require(isinstance(inventory, list), "transaction005 inventory absent")
    actual_paths = {
        path for path in TRANSACTION5.rglob("*") if path.is_file() or path.is_symlink()
    }
    recorded_paths = {Path(record["path"]) for record in inventory}
    require(actual_paths == recorded_paths, "partial transaction005 inventory")
    for record in inventory:
        authenticate(
            {key: record[key] for key in ("path", "bytes", "sha256")},
            "transaction005 inventory",
        )
    terminal_fields(
        (TRANSACTION5 / "position003/raw/terminal.txt").read_bytes()
    )
    comparison = load_json(TRANSACTION5 / "comparison.json")
    require(
        comparison
        == {
    ...
    require(
        (TRANSACTION5 / "position003/raw/trace.hex").read_bytes()
        == (TRANSACTION5 / "position003/exact_oracle/trace.hex").read_bytes()
        and (TRANSACTION5 / "position003/raw/final.hex").read_bytes()
        == (TRANSACTION5 / "position003/exact_oracle/final.hex").read_bytes(),
    ...
    mutations = (
        ...
        (
            ...
            lambda item: item["recovery_boundary"].update(
                {"generation6_publication_authorized": True}
            ),
        ),
        (
            "transaction005-execution",
            lambda item: item["recovery_boundary"].update(
                {"transactions006_025_absent": False}
            ),
        ),
    )
...
def emit_review(package: Path, output: Path) -> None:
    ...
    review = {
        "schema_version": 1,
        "kind": "ace3_transaction5_publication_recovery_independent_review",
        "status": status,
...
def main() -> None:
    ...
    print(
        "TRANSACTION005_RECOVERY_INDEPENDENT_REVIEW_WRITTEN "
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"TRANSACTION005_RECOVERY_REVIEW_REFUSED {error}") from error
