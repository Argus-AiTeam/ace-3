#!/usr/bin/env python3
"""Fail-closed validation for a Model24 r14 durable-launch package."""
...
def expected_paths(package: Path, manifest: dict[str, Any]) -> dict[str, Path]:
    ...
    return {
        ...
        "review": Path(
            f"/home/argustest/ace3-model24-r14-review-20260829-{nonce}"
        ),
        "authority": Path(
            f"/home/argustest/ace3-model24-r14-authority-20260829-{nonce}.json"
        ),
        "authority_consumed": Path(
            f"/home/argustest/ace3-model24-r14-authority-20260829-{nonce}.json.consumed"
        ),
        "output": Path(
            f"/home/argustest/ace3-model24-r14-output-20260829-{nonce}"
        ),
        "simulation_dir": Path(
            f"/home/argustest/ace3-model24-r14-output-20260829-{nonce}"
        )
...
def validate_terminal_root_value(root: Path) -> None:
    ...
    require(root.name.startswith("ace3-model24-r14-terminal-"), "terminal root name is noncanonical")
...
def validate_package_seal(package: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    require(seal["kind"] == "ace3_model24_r14_package_seal", "package seal kind mismatch")
...
def validate_contract(package: Path, manifest: dict[str, Any]) -> None:
    ...
    require(contract["kind"] == "ace3_model24_r14_durable_launch_contract", "launch contract kind mismatch")
    ...
    require(identity["task_id"] == f"ace3-model24-r14-42c895c-{identity['nonce']}", "task identity mismatch")
    paths = expected_paths(package, manifest)
    namespaces = manifest["namespaces"]
    require(set(namespaces) == set(paths) - {"base"}, "namespace fields differ from r14 schema")
...
def expected_authority(
    ...
    return {
        "schema_version": 1,
        "kind": "ace3_model24_r14_manager_exactly_once_authority",
        "nonce": manifest["identity"]["nonce"],
        "task_id": manifest["identity"]["task_id"],
...
def validate_launch_terminal(
    ...
    require(
        launch_terminal.get("kind") == "ace3_model24_r14_launch_terminal",
