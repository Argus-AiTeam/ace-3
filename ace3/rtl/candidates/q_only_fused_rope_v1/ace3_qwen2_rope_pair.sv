...
module ace3_qwen2_rope_pair (
    ...
    // Q keeps exact products through the sum; K retains its two-round path.
    wire signed [82:0] q_low_sum_w =
        $signed({low_cos_product_w[81], low_cos_product_w}) +
        $signed({neg_high_sin_product_w[81], neg_high_sin_product_w});
    wire signed [82:0] q_high_sum_w =
        $signed({high_cos_product_w[81], high_cos_product_w}) +
        $signed({low_sin_product_w[81], low_sin_product_w});
    wire [15:0] q_low_raw_w;
    wire [15:0] q_high_raw_w;
    wire q_low_saturation_w;
    wire q_high_saturation_w;
    ace3_q47_48_to_f16_rne #(.ACC_WIDTH(83)) round_q_low (
        .fixed_i(q_low_sum_w),
        .f16_o(q_low_raw_w),
        .saturation_o(q_low_saturation_w)
    );
    ace3_q47_48_to_f16_rne #(.ACC_WIDTH(83)) round_q_high (
        .fixed_i(q_high_sum_w),
        .f16_o(q_high_raw_w),
        .saturation_o(q_high_saturation_w)
    );
    wire [15:0] q_low_w = (q_low_raw_w == 16'd0 && q_low_sum_w < 0)
        ? 16'h8000 : q_low_raw_w;
    wire [15:0] q_high_w = (q_high_raw_w == 16'd0 && q_high_sum_w < 0)
        ? 16'h8000 : q_high_raw_w;
        ...
        if (!rst_ni) begin
            ...
            if (in_valid_i && in_ready_o) begin
                ...
                if (!operands_finite_w) begin
                    ...
                    low_f16_q <= is_key_i ? rotated_low_w : q_low_w;
                    high_f16_q <= is_key_i ? rotated_high_w : q_high_w;
                    saturation_q <= is_key_i ? result_saturation_w :
                        (q_low_saturation_w || q_high_saturation_w);
                end
