 ...
 */
module ace3_decoder_token_engine_q24_v1 #(
    parameter integer LAYER_INDEX = 0,
    parameter integer ACCURATE_SILU = (LAYER_INDEX >= 3),
    parameter integer RESIDUAL_WIDTH = 64,
    parameter integer RESIDUAL_FRAC_BITS = 24
) (
    ...
    input wire load_state_root_i,
    input wire signed [RESIDUAL_WIDTH-1:0] load_state_q24_i,
    input wire load_state_negzero_i,
    input wire [4:0] load_state_layer_i,
    input wire [1:0] load_state_slot_i,
    input wire [14:0] load_state_position_i,
    output wire trace_state_valid_o,
    output wire signed [RESIDUAL_WIDTH-1:0] trace_state_q24_o,
    output wire trace_state_negzero_o,
    output wire signed [RESIDUAL_WIDTH-1:0] final_state_q24_o,
    output wire final_state_negzero_o,
    output wire done_fault_o,
    output wire [3:0] done_fault_code_o,
    output wire [4:0] done_next_layer_o
);
    ...
    initial begin
        if (RESIDUAL_WIDTH != 64 || RESIDUAL_FRAC_BITS != 24)
            $fatal(1, "Q24 decoder requires residual width=64 and fraction bits=24");
    ...
    reg signed [RESIDUAL_WIDTH-1:0] residual_mem [0:895];
    reg residual_negzero_mem [0:895];
    reg signed [RESIDUAL_WIDTH-1:0] intermediate_state_mem [0:895];
    reg intermediate_negzero_mem [0:895];
    reg [1:0] state_slot_q;
    reg [14:0] state_position_q;
    reg state_load_started_q;
    reg [3:0] fault_code_q;
    reg final_sent_q, final_trace_sent_q;
    ...
    wire fault_detect_w;
    wire controller_healthy_w = (rst_ni === 1'b1) && (clear_i === 1'b0) &&
                                !fault_q && !fault_detect_w;
    ...
    wire state_load_start_ready_w, state_load_ready_w, state_load_valid_w;
    wire signed [RESIDUAL_WIDTH-1:0] state_load_value_w;
    wire state_load_negzero_w, state_load_last_w;
    wire [15:0] state_load_f16_w;
    wire [12:0] state_load_index_w;
    wire [3:0] state_load_fault_w;
    reg [3:0] state_input_fault_w;
    wire state_load_request_w = idle_w && !done_valid_q &&
        load_valid_i === 1'b1 && load_kind_i === 2'd0;
    always_comb begin
        state_input_fault_w = 4'd0;
        if (state_load_request_w) begin
            if ((^{load_state_root_i, load_state_q24_i, load_state_negzero_i,
                   load_state_layer_i, load_state_slot_i, load_state_position_i,
                   load_index_i, load_f16_i}) === 1'bx ||
                load_f16_i[14:10] == 5'h1f)
                state_input_fault_w = 4'd1;
            else if (load_state_q24_i != 0 && load_state_negzero_i)
                state_input_fault_w = 4'd2;
            else if (load_state_layer_i != LAYER_INDEX_VALUE ||
                     load_state_root_i != (LAYER_INDEX == 0) ||
                     load_state_slot_i[1] || load_state_position_i[14:7] != 0 ||
                     load_index_i != load_act_index_ext_w ||
                     load_index_i >= 13'd896 || activation_loaded_q ||
                     (load_act_q != 0 &&
                      (load_state_slot_i != state_slot_q ||
                       load_state_position_i != state_position_q)) ||
                     (load_state_root_i &&
                      (load_state_q24_i != 0 || load_state_negzero_i)))
                state_input_fault_w = 4'd5;
        end
    end
    ace3_residual_exact_grid_q24_core #(
        .VECTOR_SIZE(896), .STATE_WIDTH(RESIDUAL_WIDTH),
        .FRAC_BITS(RESIDUAL_FRAC_BITS)) state_loader (
        .clk_i(clk_i), .rst_ni(rst_ni), .clear_i(clear_i),
        .start_valid_i(state_load_request_w && !state_load_started_q &&
                       controller_healthy_w),
        .start_ready_o(state_load_start_ready_w), .element_count_i(13'd896),
        .in_valid_i(state_load_request_w && state_load_started_q &&
                    controller_healthy_w),
        .in_ready_o(state_load_ready_w),
        .in_state_q24_i(load_state_root_i ? {RESIDUAL_WIDTH{1'b0}} : load_state_q24_i),
        .in_state_negzero_i(load_state_root_i ? (load_f16_i == 16'h8000) :
                                              load_state_negzero_i),
        .in_addend_f16_i(load_state_root_i ? load_f16_i :
                        {load_state_negzero_i, 15'd0}),
        .out_valid_o(state_load_valid_w), .out_ready_i(controller_healthy_w),
        .out_state_q24_o(state_load_value_w), .out_state_negzero_o(state_load_negzero_w),
        .out_f16_o(state_load_f16_w), .out_index_o(state_load_index_w),
        .out_last_o(state_load_last_w), .out_fault_code_o(state_load_fault_w),
        .busy_o());
    wire state_view_fault_w = state_load_valid_w &&
        (state_load_fault_w != 0 ||
         state_load_f16_w !== activation_mem[state_load_index_w]);
    wire state_start_fault_w = idle_w && !done_valid_q && start_valid_i === 1'b1 &&
        (!activation_loaded_q || !n1_loaded_q || !n2_loaded_q ||
         start_cache_slot_i !== state_slot_q ||
         start_position_i !== state_position_q);
    ...
    assign load_ready_o = rst_ni && !clear_i && controller_healthy_w &&
        ...
        ((load_kind_i == 2'd0 &&
          state_load_started_q && state_load_ready_w && !activation_loaded_q &&
          load_index_i == load_act_index_ext_w &&
         ...
         (load_kind_i == 2'd2 &&
          ...
          load_index_i < 13'd896));
    assign done_valid_o = rst_ni && !clear_i && done_valid_q;
    assign done_fault_o = fault_q;
    assign done_fault_code_o = fault_code_q;
    assign done_next_layer_o = 5'(LAYER_INDEX + 1);
    ...
    wire signed [RESIDUAL_WIDTH-1:0] r1_state_w, r2_state_w;
    wire r1_negzero_w, r2_negzero_w;
    wire [3:0] r1_fault_w, r2_fault_w;
    assign r1_invalid_w = r1_fault_w != 0;
    assign r2_invalid_w = r2_fault_w != 0;
    assign r1_saturation_w = 1'b0;
    assign r2_saturation_w = 1'b0;
    wire final_pair_ready_w = (final_sent_q || final_ready_i === 1'b1) &&
                             (final_trace_sent_q || trace_ready_i === 1'b1);
    wire final_pair_transfer_w = controller_healthy_w && final_mode_w &&
                                 r2_out_valid_w && r2_result_ok_w && final_pair_ready_w;
    ace3_residual_exact_grid_q24_core #(.VECTOR_SIZE(896), .STATE_WIDTH(RESIDUAL_WIDTH), .FRAC_BITS(RESIDUAL_FRAC_BITS)) res1 (
      ...
      .in_valid_i(state_q==S_R1_IN),.in_ready_o(r1_in_ready_w),.in_addend_f16_i(o_mem[hidden_index_q]),.in_state_q24_i(residual_mem[hidden_index_q]),.in_state_negzero_i(residual_negzero_mem[hidden_index_q]),
      .out_valid_o(r1_out_valid_w),.out_ready_i(controller_healthy_w&&((state_q==S_R1_IN)||(state_q==S_R1_OUT))&&trace_free_w&&(!r1_out_valid_w||r1_result_ok_w)),.out_f16_o(r1_out_w),.out_index_o(r1_idx_w),.out_last_o(r1_last_w),.out_fault_code_o(r1_fault_w),.out_state_q24_o(r1_state_w),.out_state_negzero_o(r1_negzero_w),.busy_o(r1_busy_w));
    ace3_residual_exact_grid_q24_core #(.VECTOR_SIZE(896), .STATE_WIDTH(RESIDUAL_WIDTH), .FRAC_BITS(RESIDUAL_FRAC_BITS)) res2 (
      ...
      .in_valid_i(state_q==S_R2_IN),.in_ready_o(r2_in_ready_w),.in_addend_f16_i(down_mem[hidden_index_q]),.in_state_q24_i(intermediate_state_mem[hidden_index_q]),.in_state_negzero_i(intermediate_negzero_mem[hidden_index_q]),
      .out_valid_o(r2_out_valid_w),.out_ready_i(controller_healthy_w&&final_mode_w&&final_pair_ready_w&&(!r2_out_valid_w||r2_result_ok_w)),.out_f16_o(r2_out_w),.out_index_o(r2_idx_w),.out_last_o(r2_last_w),.out_fault_code_o(r2_fault_w),.out_state_q24_o(r2_state_w),.out_state_negzero_o(r2_negzero_w),.busy_o(r2_busy_w));
    ...
    assign fault_detect_w = rst_ni && !clear_i && !fault_q &&
       (!disabled_bias_contract_ok_w || child_busy_fault_w ||
        ...
        attention_result_fault_w || state_input_fault_w != 0 ||
        state_view_fault_w || state_start_fault_w);

    assign final_valid_o = controller_healthy_w && final_mode_w &&
                          r2_out_valid_w && r2_result_ok_w && !final_sent_q;
    assign final_state_q24_o = r2_state_w;
    assign final_state_negzero_o = r2_negzero_w;
    ...
    assign trace_valid_o = controller_healthy_w &&
        (final_mode_w ? (r2_out_valid_w && r2_result_ok_w && !final_trace_sent_q) : trace_valid_q);
    ...
    assign trace_state_valid_o = trace_valid_o &&
        (trace_stage_o == TRACE_RES1 || trace_stage_o == TRACE_RES2);
    assign trace_state_q24_o = !trace_state_valid_o ? {RESIDUAL_WIDTH{1'b0}} :
        final_mode_w ? r2_state_w : intermediate_state_mem[trace_index_q];
    assign trace_state_negzero_o = trace_state_valid_o &&
        (final_mode_w ? r2_negzero_w : intermediate_negzero_mem[trace_index_q]);
    ...
    always @(posedge clk_i or negedge rst_ni) begin
      if (!rst_ni) begin
        state_slot_q <= 0; state_position_q <= 0; state_load_started_q <= 0;
        final_sent_q <= 0; final_trace_sent_q <= 0; fault_code_q <= 0;
      end else if (clear_i) begin
        state_slot_q <= 0; state_position_q <= 0; state_load_started_q <= 0;
        final_sent_q <= 0; final_trace_sent_q <= 0; fault_code_q <= 0;
      end else begin
        if (state_load_request_w && !state_load_started_q &&
            state_load_start_ready_w && controller_healthy_w)
          state_load_started_q <= 1;
        if (load_valid_i && load_ready_o && load_kind_i == 0 && load_act_q == 0) begin
          state_slot_q <= load_state_slot_i;
          state_position_q <= load_state_position_i;
        end
        if (state_load_valid_w && controller_healthy_w) begin
          residual_mem[state_load_index_w] <= state_load_value_w;
          residual_negzero_mem[state_load_index_w] <= state_load_negzero_w;
          if (state_load_last_w) state_load_started_q <= 0;
        end
        if (((state_q == S_R1_IN) || (state_q == S_R1_OUT)) &&
            r1_out_valid_w && trace_free_w && r1_result_ok_w && controller_healthy_w) begin
          intermediate_state_mem[r1_idx_w] <= r1_state_w;
          intermediate_negzero_mem[r1_idx_w] <= r1_negzero_w;
        end
        if (final_pair_transfer_w) begin
          residual_mem[r2_idx_w] <= r2_state_w;
          residual_negzero_mem[r2_idx_w] <= r2_negzero_w;
          final_sent_q <= 0; final_trace_sent_q <= 0;
        end else begin
          if (final_valid_o && final_ready_i === 1'b1) final_sent_q <= 1;
          if (final_mode_w && trace_valid_o && trace_ready_i === 1'b1)
            final_trace_sent_q <= 1;
        end
        if (fault_detect_w) begin
          if (state_input_fault_w != 0) fault_code_q <= state_input_fault_w;
          else if (state_load_valid_w && state_load_fault_w != 0)
            fault_code_q <= state_load_fault_w;
          else if (state_view_fault_w) fault_code_q <= 6;
          else if (state_start_fault_w) fault_code_q <= 5;
          else if (r1_out_valid_w && r1_fault_w != 0) fault_code_q <= r1_fault_w;
          else if (r2_out_valid_w && r2_fault_w != 0) fault_code_q <= r2_fault_w;
          else fault_code_q <= 1;
        end
      end
    end
      ...
      if (!rst_ni) begin
        ...
        if (load_valid_i && load_ready_o) begin
          if (load_kind_i==0) begin activation_mem[load_act_q]<=load_f16_i;
            ...
            if(load_act_q==10'd895) begin load_act_q<=0; end else load_act_q<=load_act_q+10'd1; end
        ...
        if (state_load_valid_w && state_load_last_w && controller_healthy_w)
          activation_loaded_q <= 1;
        if (fault_detect_w) begin
          fault_q<=1'b1;
          state_q<=S_FAULT;
          trace_valid_q<=1'b0;
          done_valid_q<=1'b1;
          done_slot_q<=token_slot_q; done_position_q<=token_position_q;
          done_cycle_q<=cycle_q; done_stall_q<=stall_q;
          ...
          S_R2_IN:begin
            ...
            if(final_pair_transfer_w&&r2_last_w&&context_len_q[token_slot_q]=={1'b0,token_position_q})begin context_len_q[token_slot_q]<={1'b0,token_position_q}+8'd1;done_valid_q<=1;done_slot_q<=token_slot_q;done_position_q<=token_position_q;done_cycle_q<=cycle_q+32'd1;done_stall_q<=stall_q;state_q<=S_IDLE;end
          end
          S_R2_OUT:if(final_pair_transfer_w&&r2_last_w&&context_len_q[token_slot_q]=={1'b0,token_position_q})begin context_len_q[token_slot_q]<={1'b0,token_position_q}+8'd1;done_valid_q<=1;done_slot_q<=token_slot_q;done_position_q<=token_position_q;done_cycle_q<=cycle_q+32'd1;done_stall_q<=stall_q;state_q<=S_IDLE;end
