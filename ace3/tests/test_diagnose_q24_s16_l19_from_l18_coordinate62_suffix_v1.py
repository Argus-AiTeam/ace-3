"""Frozen L18 lineage and L19 surface tests; native dispatch is always mocked."""
...
from ace3.model.candidates import diagnose_q24_s16_l19_from_l18_coordinate62_suffix_v1 as d


EXPECTED_TESTS = 32


class L19SuffixPlanTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.reports = json.loads((d.SELECTED / "actual_L18_gates.json").read_text())
    ...
    def test_contract_rejects_flags_sources_and_classification_changes(self):
        for key in (*d.FLAGS, "selected_result", "selected_result_sha256", "inherits_sha256",
                    ...
                    "retained_L18_gate_passing_controls",
    ...
    def test_bounds_reject_expansion_replay_and_output_relaxation(self):
        for layer, position in ((True, 0), (18, 0), (20, 0), (19, 1)):
        ...
        for section in ("planned_bounds", "execution_bounds"):
            for key, value in (("layers", [18, 19]), ("native_L0_L18_invocations", 1),
    ...
    def test_exact_thirteen_planned_controls_and_zero_check_dispatch(self):
        ...
        self.assertEqual(bounds["planned_native_L19_P0_invocations"], 13)
        self.assertEqual(bounds["layers"], [19])
        ...
        for key in ("native_layer_invocations", "native_L0_L18_invocations", "rtl_invocations"):
    ...
    def test_result_rejects_claim_count_and_lineage_changes(self):
        ...
        for key, value in (("candidate_admitted", True), ("policy_adopted", True),
                           ...
                           ("native_L18_P0_invocations", 14),
                           ("all_L18_gate_passing_controls", list(d.CONTROLS)),
                           ("retained_L17_gate_passing_controls", list(d.PASSING)),
    ...
    def test_nine_passes_four_coordinate62_failures_and_actual_are_retained(self):
        d.check_rows(self.rows)
        self.assertEqual(self.rows[0]["control"], "actual")
        self.assertEqual(self.rows[0]["L18_status"], "FAIL")
        self.assertEqual([row["control"] for row in self.rows if row["all_L18_gates_pass"]],
                         list(d.PASSING))
        self.assertEqual(len(d.PASSING), 9)
        self.assertEqual(list(d.FAILING), ["actual", "frozen_o", "frozen_down", "frozen_o_down"])
        self.assertTrue(all(row["retained_L16_all_gates_pass"] and
                            row["retained_L17_all_gates_pass"] for row in self.rows))
        for row in self.rows:
            self.assertEqual(row["mandatory_statuses"][:18], ["PASS"] * 18)
            self.assertEqual(row["L18_S18_failure_indices"],
                             [] if row["control"] in d.PASSING else [62])

    def test_rows_reject_erased_failures_changed_passes_and_state_kv(self):
        for index, key, value in (
                (0, "L18_status", "PASS"), (0, "L18_S18_failure_indices", []),
                (0, "L18_S18_failure_indices", [63]), (0, "all_L18_gates_pass", True),
                (0, "conditional_acceptance_retained", True),
                ...
                (0, "L18_source_operand_state_KV_RTZ_checks", "FAIL"),
                (0, "prior_kv", "L18 KV"), (0, "prior_layer_kv_consumed", True),
                (0, "candidate_admitted", True), (1, "L18_status", "FAIL"),
                (1, "all_L18_gates_pass", False), (1, "L18_S18_failure_indices", [62])):
    ...
    def test_reports_preserve_all_nineteen_gates_and_actual_failure(self):
    ...
    def test_thirteen_complete_independent_izh_parents_exclude_l18_kv(self):
        for label in d.CONTROLS:
            with self.subTest(control=label), \
                    d.np.load(d.SELECTED / (label + "_L18.npz"), allow_pickle=False) as arrays:
    ...
    def test_parent_rejects_fp16_only_and_spliced_state(self):
        with d.np.load(d.SELECTED / "actual_L18.npz", allow_pickle=False) as saved:
    ...
    def test_independent_e1c2af4f8e0b_round_one_review_required(self):
    ...
    def test_reference_follows_original_input_l19_chain(self):
        self.assertIs(d.check_reference(self.extension), self.extension["layers"]["19"])
        for key in ("binary64", "fp16"):
            self.assertEqual(self.extension["layers"]["19"][f"input_{key}"],
                             self.extension["layers"]["18"][key])

    def test_reference_rejects_reanchoring_kv_and_substitution(self):
        for layer, key in (("19", "input_binary64"), ("19", "input_fp16"), ("19", "prior_kv"),
                           ("18", "input_binary64"), ("17", "input_binary64"),
                           ("16", "input_binary64"), ("15", "input_binary64")):
        ...
        for key in ("binary64", "fp16"):
            extension = deepcopy(self.extension)
            extension["layers"]["19"][key]["sha256"] = "0" * 64
            with self.subTest(key=key), self.assertRaises(ValueError):
                d.check_reference(extension)
    ...
    def test_cli_check_only_calls_validation(self):
        with patch.object(d, "validate", return_value={"L19_status": "NOT_EXECUTED"}) as check, \
                patch("sys.stdout", new_callable=io.StringIO) as output:
            self.assertEqual(d.main(["--check"]), 0)
        check.assert_called_once_with()
        self.assertEqual(json.loads(output.getvalue()), {"L19_status": "NOT_EXECUTED"})
    ...
    def test_execution_interface_preserves_bounds_and_non_admission(self):
        ...
        self.assertEqual(bounds["native_L19_P0_invocations"], 13)
    ...
    def controls(self):
        parents = {}
        for label in d.CONTROLS:
            with d.np.load(d.SELECTED / (label + "_L18.npz"), allow_pickle=False) as arrays:
    ...
    def test_mocked_stage_route_uses_only_l19_original_reference_and_all_gates(self):
        ...
        with patch.object(d.producer.native, "stages", side_effect=stages) as native, \
            ...
            _, locals_, reports = d.execute_layer(tensors, state, trajectory, reference)
        native.assert_called_once()
        self.assertEqual(len(locals_), 18)
        self.assertEqual(local.call_count, 17)
        self.assertTrue(all(call.args[3] == 19 for call in local.call_args_list))
        self.assertEqual([report["node"] for report in reports], [[19, 0, s] for s in range(19)])
        ...
        for sequence in ([], [1], list(range(18)), list(range(19)) + [19]):
            with self.subTest(sequence=sequence), \
                ...
                d.execute_layer(tensors, state, trajectory, reference)
